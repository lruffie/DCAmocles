...
class Bot:
    ...
    def checks(self):
        ...
        # print('Checks for : ', self.symbol)
        order_tracked = self.track_order()
        print('type :', order_tracked['type'], ' price :', order_tracked['price'], order_tracked['status'], order_tracked['side'], self.side)
        ...
        delta = float(self.delta)
        if self.side == "SELL" :
            if float(order_tracked['price']) >= level*(1-delta/100)*1.001 or float(order_tracked['price']) <= level*(1-delta/100)*0.999:
                ...
                if order_tracked['type'] == 'MARKET':
                    try :
                        ...
                        if err.error_message == 'Stop price would trigger immediately.' :
                            ...
                            order=self.place_order_market()
                            print(self.side)
                        return ("Sucessful update of orders, new order :", order) 
                else :
                    self.cancel_order()
                    order = self.place_order()
                    return ("Sucessful update of orders, new order :", order) 
        ...
        if self.side == "BUY" :
            if float(order_tracked['price']) >= self.level*(1+self.delta/100)*1.001 or float(order_tracked['price']) <= self.level*(1+self.delta/100)*0.999:
        ...
        if float(order_tracked['origQty']) >= self.amount*1.01 or float(order_tracked['origQty']) <= self.amount*0.99:
            print('amount_update')
            if order_tracked['type'] == 'MARKET':
                    order = self.place_order()
